...
import { Play, Pause, SlidersHorizontal, X, RotateCcw, Home, ChevronLeft, ChevronRight, Music } from "lucide-react"
                ...
                {MOOD_SLIDER_CONFIG.map((mood, i) => (
                  <div key={mood.key} className="space-y-2 p-4 rounded-2xl bg-card">
                    <div className="flex items-center justify-between">
                      <span
                        className={`text-sm font-bold ${i % 3 === 0 ? "text-primary" : i % 3 === 1 ? "text-[#4d7cfe]" : "text-[#a855f7]"}`}
                      >
                        {mood.leftLabel}
                      </span>
                      <span
